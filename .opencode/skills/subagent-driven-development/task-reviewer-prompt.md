...
Dispatch via the task tool:
  ...
  prompt: |
    ...
    **Diff file:** [DIFF_FILE]
    ...
    unchanged code or spans tasks), report it as a "⚠️ Cannot verify from
    diff" item instead of broadening your search.
...
**Placeholders:**
- `[BRIEF_FILE]` — REQUIRED: the task brief file (`scripts/task-brief.ps1 PLAN N` prints the path; same file the implementer worked from)
...
- `[DIFF_FILE]` — REQUIRED: the path the controller wrote the review package to (`scripts/review-package.ps1 PLAN_FILE BASE HEAD` prints the unique path it wrote; the package never enters the controller's context)
